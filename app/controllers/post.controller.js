...
exports.deletePost = (req, res) => {
  ...
  if (!postId) {
    return res.status(BADREQUEST).send({ isSuccessful: false, message: "Post ID is required" });
  }
    Users.deletePost(postId, (err, data) => {
      if (err) return res.status(INTERNALSERVERERROR).send({ isSuccessful: false, message: "Error deleting post" });
      return res.status(HTTP200OK).send({ isSuccessful: true, message: "Post deleted" });
    });
};
