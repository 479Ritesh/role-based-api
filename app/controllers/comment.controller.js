...
exports.createComment = (req, res) => {
  ...
  if (!content || !postId || !userId) {
    return res
      .status(400)
      .send({ message: "Content, userId and postId are required." });
  }
  ...
  Users.createComment(comment, (err, fullComment) => {
    if (err || !fullComment) {
      ...
      return res
        ...
        .send({
          isSuccessful: false,
          message: "Error creating comment or fetching data",
        });
    }
  ...
  Users.getCommentsByPost(postId, (err, data) => {
    if (err)
      return res
        ...
        .send({ isSuccessful: false, message: "Error fetching comments" });
...
exports.updateComment = (req, res) => {
  ...
  if (!commentId || !content) {
    return res
      .status(400)
      .send({ isSuccessful: false, message: "Missing commentId or content" });
  }
  ...
  Users.updateComment(commentId, content, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        return res
          .status(404)
          .send({ isSuccessful: false, message: "Comment not found" });
      }
      return res
        .status(500)
        .send({ isSuccessful: false, message: "Error updating comment" });
    }

    return res
      .status(200)
      .send({ isSuccessful: true, message: "Comment updated successfully" });
  });
};
...
exports.deleteComment = (req, res) => {
  ...
  Users.deleteComment(commentId, (err, data) => {
    if (err)
      return res
        .status(INTERNALSERVERERROR)
        .send({ isSuccessful: false, message: "Error deleting comment" });
    return res
      .status(HTTP200OK)
      .send({ isSuccessful: true, message: "Comment deleted" });
  });
};
